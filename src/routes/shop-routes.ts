...
shopRoutes.delete("/:id", function(req, res) {
    //use id parameter to delete corresponding shop object from shops array
    let inputId: number = Number.parseInt(req.params.id);
    //options:
    //loop through array
        //if input id matchs the shop[i]
            // shops.splice[i];
//OR
    // use find() - const found = shops.find(shop => shop.id === <id#>);
    // or findindex and splice
